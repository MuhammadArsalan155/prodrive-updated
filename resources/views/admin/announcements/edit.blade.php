            ...
            <div class="card-body">
                <form action="{{ route('admin.announcements.update', $announcement) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        ...
                        <textarea class="form-control" id="editor" name="content">{{ old('content', $announcement->content) }}</textarea>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        @if($announcement->attachment)
                            <div class="attachment-current">
                                ...
                                <a href="{{ route('admin.announcements.download', $announcement) }}" class="attachment-download" target="_blank">
                                    <i class="fas fa-file-download mr-1"></i> {{ basename($announcement->attachment) }}
                                </a>
                                <div class="custom-control custom-checkbox mt-3">
                                    <input type="checkbox" class="custom-control-input" id="remove_attachment" name="remove_attachment">
                        ...
                        <small class="form-text text-muted">Max file size: 10MB. Leave empty to keep current attachment.</small>
                    </div>
                    ...
                    <div class="form-row">
                        ...
                        <div class="form-group col-md-6">
                            ...
                            <select class="form-control" id="target_type" name="target_type" required>
                                ...
                                <option value="role" {{ old('target_type', $announcement->target_type) == 'role' ? 'selected' : '' }}>Specific Roles</option>
                                <option value="user" {{ old('target_type', $announcement->target_type) == 'user' ? 'selected' : '' }}>Specific Users</option>
                            </select>
                        </div>
                    </div>
                            ...
                            @foreach($roles as $role)
                                <option value="{{ $role->id }}" 
                                    {{ (is_array(old('roles')) && in_array($role->id, old('roles'))) || 
                                       (old('roles') === null && $announcement->targetRoles->contains($role->id)) ? 'selected' : '' }}>
                                        ...
                                        @foreach($users as $user)
                                            <option value="{{ $user->id }}" 
                                                {{ (is_array(old('users')) && in_array($user->id, old('users'))) || 
                                                   (old('users') === null && $announcement->targetUsers->contains($user->id)) ? 'selected' : '' }}>
                                    ...
                                    <select class="form-control select2-multiple" id="students" name="students[]" multiple>
                                        @foreach($students as $student)
                                            <option value="{{ $student->id }}" 
                                                {{ (is_array(old('students')) && in_array($student->id, old('students'))) || 
                                                   (old('students') === null && $announcement->targetStudents->contains($student->id)) ? 'selected' : '' }}>
                                    ...
                                    <select class="form-control select2-multiple" id="instructors" name="instructors[]" multiple>
                                        @foreach($instructors as $instructor)
                                            <option value="{{ $instructor->id }}" 
                                                {{ (is_array(old('instructors')) && in_array($instructor->id, old('instructors'))) || 
                                                   (old('instructors') === null && $announcement->targetInstructors->contains($instructor->id)) ? 'selected' : '' }}>
                                                {{ $instructor->name ?? $instructor->first_name . ' ' . $instructor->last_name }} ({{ $instructor->email }})
                                            </option>
                                        @endforeach
                                    </select>
                                </div>
...
<script>
    $(document).ready(function() {
        ...
        $('.custom-file-input').on('change', function() {
            ...
            $(this).next('.custom-file-label').addClass("selected").html(fileName || "Choose new file");
        });
    });
</script>
@endsection